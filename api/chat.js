...
module.exports=async function(req,res){try{
 ...
 const p=await fetch(SB_URL+'/rest/v1/beta_profiles?id=eq.'+encodeURIComponent(sender)+'&select=id,role,display_name&limit=1',{headers:headers()});
 const profiles=await p.json();if(!Array.isArray(profiles)||!profiles[0]||profiles[0].role!==role){res.status(403).json({error:'profile not registered'});return}
 const r=await fetch(SB_URL+'/rest/v1/rpc/cg_beta_chat_insert',{method:'POST',headers:headers(),body:JSON.stringify({p_sender:sender,p_sender_name:name,p_body:body})});const text=await r.text();res.status(r.status).setHeader('Cache-Control','no-store').send(text);

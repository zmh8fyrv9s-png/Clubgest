...
module.exports=async function(req,res){
 try{
  ...
  const b=req.body||{},role=b.role||'',sender=b.sender||'',body=String(b.body||'').trim().slice(0,2000),name=String(b.sender_name||'').slice(0,120);
  if(DEMO_UIDS[role]!==sender||!body){res.status(400).json({error:'invalid chat data'});return}

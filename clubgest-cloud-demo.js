...
async function pull(){if(pulling)return;pulling=true;try{const r=await fetch(URL+'&select=state,updated_at',{headers:headers()});if(!r.ok)return;const rows=await r.json();const remote=rows&&rows[0]&&rows[0].state;if(!remote||!remote.teams)return;let local=null;try{local=JSON.parse(localStorage.getItem(LOCAL)||'null')}catch(e){};const rt=rows[0].updated_at||'';if(local&&local.__cloudUpdatedAt&&new Date(local.__cloudUpdatedAt)>=new Date(rt))return;remote.__cloudUpdatedAt=rt;rawSet.call(localStorage,LOCAL,JSON.stringify(remote));if(typeof window.render==='function')window.render()}catch(e){}finally{pulling=false}}
async function push(value){if(saving)return;saving=true;try{const state=JSON.parse(value);delete state.__cloudUpdatedAt;const r=await fetch(URL,{method:'PATCH',headers:headers({Prefer:'return=minimal'}),body:JSON.stringify({state,updated_at:new Date().toISOString()})});if(r.ok){const local=JSON.parse(value);local.__cloudUpdatedAt=new Date().toISOString();rawSet.call(localStorage,LOCAL,JSON.stringify(local))}}catch(e){}finally{saving=false}}
